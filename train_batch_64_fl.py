...
from dataProcess import dataGen, valGen
from model_1 import VGG
...
def focal_loss(labels, logits, alpha=0.25, gamma=2):
    ...
    prob = tf.clip_by_value(prob, epsilon, 1.0 - epsilon)
    zeros = tf.zeros_like(prob, dtype=tf.float32)
    pos_corr = tf.where(prob > zeros, labels - prob, zeros)
    neg_corr = tf.where(prob > zeros, zeros, prob)
    fl_loss = - alpha * (pos_corr**gamma) * tf.log(prob) - (1.0 - alpha) * (neg_corr**gamma) * tf.log(1.0 - prob)
